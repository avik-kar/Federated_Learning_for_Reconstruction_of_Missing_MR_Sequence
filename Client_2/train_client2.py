...
from dataloader_client2 import *
...
def train(args):
    ...
    if 'Discriminators_client2.pt' in list_models and args.adv_train == 'y':
        discriminators = torch.load(args.modelpath+'Discriminators_client2.pt')
    ...
    if 'rx_client2.pt' in list_models:
        received_models = torch.load(args.modelpath+'rx_client2.pt')
    ...
    for epoch in range(args.epoch):
        ...
        for data_ in tq(trainloader):
            all_seq, _flair, _t1ce, _t2 = data_
            
            # send input tensors to cuda if available
            if args.use_gpu:
                all_seq, _flair, _t1ce, _t2 = all_seq.cuda().float(), _flair.cuda().float(), _t1ce.cuda().float(), _t2.cuda().float()
            ...
            alpha = 0.001
            if args.adv_train == 'y':
                Total_Loss = (Rec_loss_flair + Rec_loss_t1ce + Rec_loss_t2) - alpha*(D1_loss + D2_loss + D3_loss)
            else:
                Total_Loss = (Rec_loss_flair + Rec_loss_t1ce + Rec_loss_t2)
            
            # update generator networks
            Total_Loss.backward()
            optimizer_S.step()
            optimizer_P.step()
            Recon_Loss += (Rec_loss_flair + Rec_loss_t1ce + Rec_loss_t2).item()
    ...
    # store model weights
    transportables = {'SNet': SNet.state_dict(), 'PNet': PNet.state_dict(), 'client_info': ['flair', 't1ce', 't2']}
    torch.save(transportables, args.modelpath+'tx_client2.pt')
    
    if args.adv_train == 'y':
        discriminators = {'D1': D1Net.state_dict(), 'D2': D2Net.state_dict(), 'D3': D3Net.state_dict()}
        torch.save(discriminators, args.modelpath+'Discriminators_client2.pt')
    ...
    del trainReconLoss, transportables, SNet, PNet, input_, all_seq, _flair, _t1ce, _t2, ip_agnostic_feat, rec_images, ip_agnostic_feat_flair, ip_agnostic_feat_t1ce, ip_agnostic_feat_t2
