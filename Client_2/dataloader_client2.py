...
class dataLoader(data.Dataset):
    ...
    def __init__(
        ...
        for patient in self.patientList:
            self.files += os.listdir(self.path+patient+'/flair/')
    ...
    def __getitem__(self, index):
        ...
        if self.augmentations is not None:
            im_flair, im_t1ce, im_t2 = self.augmentations(im_flair, im_t1ce, im_t2)
        if self.is_transform:
            im_flair, im_t1ce, im_t2 = self.transform(im_flair, im_t1ce, im_t2)
        
        mis_seq = torch.zeros(im_flair.shape)
        
        return torch.cat((im_flair, mis_seq, im_t1ce, im_t2), 0), torch.cat((mis_seq, mis_seq, im_t1ce, im_t2), 0), torch.cat((im_flair, mis_seq, mis_seq, im_t2), 0), torch.cat((im_flair, mis_seq, im_t1ce, mis_seq), 0)

    def transform(self, im_flair, im_t1ce, im_t2):
