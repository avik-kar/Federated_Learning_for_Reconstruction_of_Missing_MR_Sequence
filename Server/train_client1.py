...
from dataloader_client1 import *

def ch_shuffle(x):
    """This function returns channel shuffled version of input
    and corresponding labels of shuffling"""
...
def train(args):
    ...
    if 'Discriminators_client1.pt' in list_models and args.adv_train == 'y':
        discriminators = torch.load(args.modelpath+'Discriminators_client1.pt')
    ...
    if 'rx_client1.pt' in list_models:
        received_models = torch.load(args.modelpath+'rx_client1.pt')
    ...
    for epoch in range(args.epoch):
        ...
        for data_ in tq(trainloader):
            all_seq, _t1, _t1ce, _t2 = data_
            
            # send input tensors to cuda if available
            if args.use_gpu:
                all_seq, _t1, _t1ce, _t2 = all_seq.cuda().float(), _t1.cuda().float(), _t1ce.cuda().float(), _t2.cuda().float()
            ...
            if args.adv_train == 'y':
                ...
                Ch_shuffle_data_1 = torch.cat((rec_images[:,1,:,:].unsqueeze(1), all_seq[:,1,:,:].unsqueeze(1)),1)
                Ch_shuffle_data_2 = torch.cat((rec_images[:,2,:,:].unsqueeze(1), all_seq[:,2,:,:].unsqueeze(1)),1)
                Ch_shuffle_data_3 = torch.cat((rec_images[:,3,:,:].unsqueeze(1), all_seq[:,3,:,:].unsqueeze(1)),1)
            ...
            if args.enforce_similarity == 'y':
                ...
                optimizer_P.zero_grad()
                ip_agnostic_feat_t2 = PNet(Variable(_t2))
                feat_loss_t2 = criterion_feat_t2(ip_agnostic_feat_t2, ip_agnostic_feat)
                feat_loss_t2.backward()
                optimizer_P.step()
            ...
            Rec_loss_t2 = criterion_Rec_t2(rec_images[:,3,:,:],all_seq[:,3,:,:])
            
            # calculate total loss and store
            alpha = 0.001
            if args.adv_train == 'y':
                Total_Loss = (Rec_loss_t1 + Rec_loss_t1ce + Rec_loss_t2) - alpha*(D1_loss + D2_loss + D3_loss)
            else:
                Total_Loss = (Rec_loss_t1 + Rec_loss_t1ce + Rec_loss_t2)
            
            # update generator networks
            Total_Loss.backward()
            optimizer_S.step()
            optimizer_P.step()
            Recon_Loss += (Rec_loss_t1 + Rec_loss_t1ce + Rec_loss_t2).item()
    ...
    # store model weights
    transportables = {'SNet': SNet.state_dict(), 'PNet': PNet.state_dict(), 'client_info': ['t1', 't1ce', 't2']}
    torch.save(transportables, args.modelpath+'tx_client1.pt')
    
    if args.adv_train == 'y':
        discriminators = {'D1': D1Net.state_dict(), 'D2': D2Net.state_dict(), 'D3': D3Net.state_dict()}
        torch.save(discriminators, args.modelpath+'Discriminators_client1.pt')
    ...
    del trainReconLoss, transportables, SNet, PNet, input_, all_seq, _t1, _t1ce, _t2, ip_agnostic_feat, rec_images, ip_agnostic_feat_t1, ip_agnostic_feat_t1ce, ip_agnostic_feat_t2
